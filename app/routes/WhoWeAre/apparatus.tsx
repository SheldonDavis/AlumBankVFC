//* IMPORTS
import type { Route } from './+types/apparatus';
...
//import page data
import { Apparatus_Data } from '~/database/WhoWeAre/Apparatus_Data';
...
export async function clientLoader({ params }: Route.LoaderArgs) {
  // const q = query(collection(db, 'Pages'), where(documentId(), '==', `Apparatus`), limit(1));
  // const querySnapshot = await getDocs(q);
  // const tempData = querySnapshot.docs[0].data().elements;
  // return await { elements: tempData };
}

export default function Apparatus({ loaderData }: Route.ComponentProps) {
  return (
    <>
      <h1>Apparatus</h1>
