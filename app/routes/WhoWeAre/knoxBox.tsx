//* IMPORTS
import type { Route } from './+types/knoxBox';
...
//import page data
import { KnoxBox_Data } from '~/database/WhoWeAre/KnoxBox_Data';
...
export async function clientLoader({ params }: Route.LoaderArgs) {
  // const q = query(collection(db, 'Pages'), where(documentId(), '==', `Knox Box Info`), limit(1));
  // const querySnapshot = await getDocs(q);
  // const tempData = querySnapshot.docs[0].data().elements;
  // return await { elements: tempData };
}

export default function KnoxBox({ loaderData }: Route.ComponentProps) {
  return (
    <>
      <h1>Knox Box Info</h1>
      {KnoxBox_Data.elements.map((block, index) => {
