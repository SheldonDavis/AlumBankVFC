...
//import page data
import { Auctions_Data } from '~/database/Activities/Auctions_Data';
...
export async function clientLoader({ params }: Route.LoaderArgs) {
  // const q = query(collection(db, 'Pages'), where(documentId(), '==', `Auctions`), limit(1));
...
export default function Auctions({ loaderData }: Route.ComponentProps) {
  return (
    <>
      <h1>Auctions</h1>
      {Auctions_Data.elements.map((block, index) => {
